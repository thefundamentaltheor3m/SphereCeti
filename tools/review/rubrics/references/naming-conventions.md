...
- `_def`: restates a definition (`foo = <its definition>`), typically for rewriting.
- `_apply`: evaluates a function or bundled map at an argument (`foo x = …`).
- `_iff`: the statement is an `Iff`; a lemma proving only one direction must not carry it.
- `_eq`: the statement is an equality; a lemma proving only an inequality must not carry it.
